...
def transliterate_word_simple(model, roman_word, roman_vocab, devanagari_vocab, device, max_length=50):
    """Alternative simple approach using model's forward method"""
    model.eval()
    ...
    with torch.no_grad():
        ...
        # Create a target tensor with just SOS and padding
        sos_idx = devanagari_vocab.char2idx.get("<sos>", 1)
        pad_idx = devanagari_vocab.char2idx.get("<pad>", 0)
        
        # Create target with SOS + padding for the rest
        trg_tensor = torch.full((1, max_length), pad_idx, device=device)
        trg_tensor[:, 0] = sos_idx
        
        # Create masks
        src_key_padding_mask = make_src_key_padding_mask(src_tensor, pad_idx=pad_idx)
        tgt_key_padding_mask = make_src_key_padding_mask(trg_tensor, pad_idx=pad_idx)
        tgt_mask = generate_square_subsequent_mask(trg_tensor.size(1), device=device)
        ...
        # Get predictions (ignore SOS position)
        predictions = output.argmax(-1)[0, 1:]  # Remove batch dim and SOS
        
        # Convert to list and stop at EOS
        decoded_indices = []
        eos_idx = devanagari_vocab.char2idx.get("<eos>", 2)
...
def generate_predictions(model, roman_vocab, dev_vocab, device, test_data, use_beam=True, beam_width=5):
    ...
    predictions = {}
    for i, roman_word in enumerate(test_data.keys()):
        try:
            if use_beam:
            ...
            if not pred:
                pred = transliterate_word_simple(model, roman_word.lower(), roman_vocab, dev_vocab, device)
