...
def beam_search_decode(model, roman_word, roman_vocab, devanagari_vocab, device, beam_width=3, max_length=50):
    """
    Beam search decoding with k=3
    FIXED: Proper hidden state handling for bidirectional encoder
    """
    model.eval()
    ...
    with torch.no_grad():
        ...
        # FIX: Properly initialize decoder hidden state for bidirectional encoder
        if model.encoder.lstm.bidirectional:
            num_layers = model.encoder.num_layers
            hidden_dim = model.encoder.hidden_dim
            batch_size = 1
            
            # Reshape encoder hidden states: (num_layers * 2, batch_size, hidden_dim)
            ...
            last_forward = encoder_hidden_reshaped[-1, 0]  # (batch_size, hidden_dim)
            last_backward = encoder_hidden_reshaped[-1, 1] # (batch_size, hidden_dim)
        ...
        # Initialize beams: (sequence, log_prob_score, hidden_state, cell_state)
        start_token = devanagari_vocab.char2idx['<sos>']
        beams = [([start_token], 0.0, decoder_hidden, decoder_cell)]
        ...
        for step in range(max_length):
            ...
            for seq, log_prob, hidden, cell in beams:
                # Stop expansion if sequence ended with <eos>
                if seq[-1] == devanagari_vocab.char2idx['<eos>']:
                ...
                # Get probabilities and convert to log space
                ...
                for i in range(beam_width):
                    ...
                    all_candidates.append((new_seq, new_log_prob, new_hidden, new_cell))
            
            # Sort all candidates by score (higher log_prob = better)
            ...
            if all(beam[0][-1] == devanagari_vocab.char2idx['<eos>'] for beam in beams):
                break
        
        # Return the candidate with highest log_prob
        best_sequence, best_score, _, _ = beams[0]
        
        # Remove <sos> and <eos> tokens
        filtered_seq = [
            idx for idx in best_sequence 
            if idx not in [devanagari_vocab.char2idx['<sos>'], devanagari_vocab.char2idx['<eos>']]
        ]
        
        return devanagari_vocab.decode(filtered_seq)

def transliterate_word(model, roman_word, roman_vocab, devanagari_vocab, device, max_length=50, use_beam_search=True):
    """Transliterate a single word using beam search (default) or greedy decoding"""
    if use_beam_search:
        return beam_search_decode(model, roman_word, roman_vocab, devanagari_vocab, device, beam_width=3, max_length=max_length)
    else:
        # Fallback to greedy decoding (your existing working code)
        model.eval()
        with torch.no_grad():
            # Encode input
            roman_chars = roman_vocab.split_roman_word(roman_word)
            ...
            # Initialize decoder (using the same logic as in your working code)
            if model.encoder.lstm.bidirectional:
                num_layers = model.encoder.num_layers
                hidden_dim = model.encoder.hidden_dim
                batch_size = 1
...
def compare_decoding_methods(model, roman_vocab, devanagari_vocab, device):
    """Compare beam search vs greedy decoding on sample words"""
    print("\n" + "="*50)
    print("🔄 BEAM SEARCH vs GREEDY DECODING COMPARISON")
    print("="*50)
    
    test_words = [
        "school", "technology", "computer", "mobile", "namaste"
    ]
    
    print(f"{'Word':<12} | {'Greedy':<15} | {'Beam Search':<15}")
    print("-" * 50)
    
    for word in test_words:
        try:
            greedy_pred = transliterate_word(model, word, roman_vocab, devanagari_vocab, device, use_beam_search=False)
            beam_pred = transliterate_word(model, word, roman_vocab, devanagari_vocab, device, use_beam_search=True)
            
            marker = " ⭐" if greedy_pred != beam_pred else ""
            print(f"{word:<12} | {greedy_pred:<15} | {beam_pred:<15}{marker}")
        except Exception as e:
            print(f"{word:<12} | ERROR: {str(e)[:30]}...")

def test_transliteration(model, roman_vocab, devanagari_vocab, device):
    """Test the model on some sample words using beam search"""
    print("\n" + "="*50)
    print("🧪 TRANSLITERATION TEST (Beam Search k=3)")
    print("="*50)
    ...
    for word in test_words:
        try:
            prediction = transliterate_word(model, word, roman_vocab, devanagari_vocab, device, use_beam_search=True)
            print(f"  {word:15} → {prediction}")
        except Exception as e:
            print(f"  {word:15} → ERROR: {str(e)[:30]}...")

def create_results_xml(test_data, model, roman_vocab, devanagari_vocab, device, output_path):
    """Create XML file in NEWS format with model predictions using beam search"""
    doc = Document()
    ...
    results.setAttribute('RunID', 'LSTM_Seq2Seq_Beam')
    results.setAttribute('RunType', 'automatic')
    results.setAttribute('Comments', 'LSTM with Attention Model + Beam Search (k=3)')
    doc.appendChild(results)
    
    # Process ALL test examples (no limit)
    print(f"\n📊 Generating predictions for {len(test_data)} examples using Beam Search (k=3)...")
    print("⚠️  This will be slower but should give better accuracy")
    ...
    for i, (roman_word, devanagari_ref) in enumerate(test_data):
        try:
            ...
            # Get model prediction using BEAM SEARCH
            prediction = transliterate_word(model, roman_word, roman_vocab, devanagari_vocab, device, use_beam_search=True)
            ...
            if (i + 1) % 100 == 0:
                print(f"  Processed {i + 1}/{len(test_data)} examples ({successful} successful)")
        except Exception as e:
            if (i + 1) % 100 == 0:
                print(f"  Processed {i + 1}/{len(test_data)} examples ({successful} successful, {i+1-successful} failed)")
    
    # Write to file
    with codecs.open(output_path, 'w', 'utf-8') as f:
        f.write(doc.toprettyxml(indent='  '))
    
    print(f"✅ Predictions saved to: {output_path} (Success rate: {successful}/{len(test_data)})")
...
def main():
    ...
    checkpoint_path = 'saved_models/best_model_lstm_ortho.pth'
    output_xml = 'model_predictions_beam_ortho.xml'  # Different name to distinguish
    ...
    compare_decoding_methods(model, roman_vocab, devanagari_vocab, device)
    
    # Test transliteration on sample words using beam search
    test_transliteration(model, roman_vocab, devanagari_vocab, device)
    
    # Load ALL test data (no limits)
    test_data = load_test_data(test_file)
    
    # Generate predictions XML for ALL examples using BEAM SEARCH
    ...
    print(f"3. Expected improvement: ACC ~0.39 → 0.45-0.50")
